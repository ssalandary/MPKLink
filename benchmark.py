...
def benchmark(service: str, test_file: str) -> tuple[int, float]:
    calculator_service = os.path.join(os.getcwd(), 'services', service, 'request-calculator')
    ...
    subprocess.call(['cargo', 'clean', '--manifest-path', f'{calculator_service}/Cargo.toml'], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    subprocess.call(['cargo', 'clean', '--manifest-path', f'{manager_service}/Cargo.toml'], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    if service == 'os-pipe':
        subprocess.call(['rm', '-f', '/tmp/request-pipe-request', '/tmp/request-pipe-response'], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    elif service == 'shared-memory':
        subprocess.call(['rm', '-f', '/tmp/request.shm', '/tmp/response.shm'], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    

    # Build the calculator and manager services
    log('[*] INFO: Building calculator and manager services...')
    subprocess.call(['cargo', 'build', '--release', '--manifest-path', f'{calculator_service}/Cargo.toml'], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    subprocess.call(['cargo', 'build', '--release', '--manifest-path', f'{manager_service}/Cargo.toml'], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)

    # Start the calulator "server"
    log('[*] INFO: Starting calculator server...')
    p_calculator = subprocess.Popen([f'{calculator_service}/target/release/request-calculator', test_file], stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    time.sleep(2)
    ...
    p_manager = subprocess.Popen([f'{manager_service}/target/release/request-manager', test_file], stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL)
...
# TODO: Fix bug for shared-memory benchmark. Currently consuming too much memory causing a crash.
SERVICES = ['os-pipe', 'unix-domain-sockets']
TESTS = sorted([test for test in os.listdir('tests') if test.endswith('.in')])
BENCHMARK_DIR = os.path.join(os.getcwd(), 'benchmark', datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
